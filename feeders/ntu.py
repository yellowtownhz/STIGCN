...
class NTU(Feeder):
    def __init__(self, data_path, label_path,
                 ...
                 frame_null=0, frame_delay=0, horizontal_flip=0,
                 use_mmap=True, trim=False, sampling_rate=1):
        ...
        :param trim: If true, make every class contain same number of samples.
        """
    ...
    def load_data(self):
        ...
        self.data = self.data[:, :, ::self.sampling_rate, :, :]
        if self.debug:
    ...
    def trim_sample(self):
        ...
        for index, label in enumerate(self.label):
            factory[label].append(index)
        least_num = min([len(x) for x in factory.values()])
        select = []
        for label, sub in factory.items():
            select += sub
        self.label = [self.label[i] for i in select]
        self.sample_name = [self.sample_name[i] for i in select]
